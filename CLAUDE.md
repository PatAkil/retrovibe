...
| juice.ts | `createJuice()` → `.shake/.flash/.hitStop/.frozen/.update/.preRender/.postRender` | Screen shake, flash, hit-stop. Order: clear → `preRender` → world → `postRender` → CRT. The shake translate is rounded to whole logical pixels, so dither fields and 1-px keylines never resample sub-pixel mid-shake. `flash`'s overlay peaks at 0.65 alpha (never full-opacity wash), holds that peak for the first 20% of the duration, then fades on a quadratic ease-out over the rest, so the burst/shake/frozen tableau underneath stays visible through the flash |
