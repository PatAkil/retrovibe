...
This recipe draws collision, not pictures: a `Rect` list is geometry, and filling each rect with one flat color is what makes a correct platformer look like a demo. The worked example for platformer art is the frozen fixture `verification/cave-hopper/game/main.ts` (read it; never run or modify a fixture). Take from it: **terrain slabs** are `drawBevel(ctx, x, y, w, h, PAL[2], PAL[3], PAL[1])` with a sparse `fillDither` rock face and a brighter dithered **lit top lip** — the lip is what tells the player where the standable surface is; the **pit** is `fillBands(ctx, PIT.x, y, PIT.w, h, [PAL[1], PAL[0], '#000000'])`, so the gap falls away into depth instead of reading as a seam between two grounds; **spikes** are a fang sprite with a bright tip, an orange upper and a dark-red lower, with an authored dark outline so the hazard never shares a value with the slab it sits on; **coins** are a 2-frame `frameIndex` spin, the **flag** a 2-frame flutter, and the **player** an outlined multi-tone sprite with a walk cycle and `flipSprite` facing. The rules behind all of that live in **ensuring-arcade-visuals** §3b (sprite craft) and §7 (backgrounds, surfaces, title).
