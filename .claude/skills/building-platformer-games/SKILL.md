...
## Terrain art — the worked example

This recipe draws collision, not pictures: a `Rect` list is geometry, and filling each rect with one flat color is what makes a correct platformer look like a demo. The worked example for platformer art is the frozen fixture `verification/cave-hopper/game/main.ts` (read it; never run or modify a fixture). Take from it: **terrain slabs** are `drawBevel(ctx, x, y, w, h, PAL[2], PAL[2], PAL[1])` with a sparse `fillDither` rock face and a brighter dithered **lit top lip** (`fillDither(ctx, x + 1, y + 1, w - 2, 1, PAL[2], PAL[5], 'sparse')`) — the lip is what tells the player where the standable surface is, and both its tones stay off the hazard's tones, or the spikes read as part of the terrain; the **pit** is `fillBands(ctx, PIT.x, y, PIT.w, h, [PAL[1], PAL[0], PAL[0]])`, so the gap falls away into the palette's darkest tone (on SUNSET that is a deep plum, deliberately not black) instead of reading as a seam between two grounds; **spikes** are a fang sprite with a bright tip (`PAL[5]`) over a `PAL[4]` body, with the darkest tone only as a 1-px base shadow and an authored dark outline, so the hazard never shares a value — or a tone — with the slab it sits on; **coins** are a 2-frame `frameIndex` spin, the **flag** a 2-frame flutter, and the **player** an outlined multi-tone sprite with a walk cycle and `flipSprite` facing. The rules behind all of that live in **ensuring-arcade-visuals** §3b (sprite craft) and §7 (backgrounds, surfaces, title).
